...
class Model(nn.Module):
    ...
    def train(
        self,
        trainer=None,
        additional_file_path: Union[str, Path] = None,
        **kwargs,
    ):
        """
        ...
        Args:
            ...
            additional_file_path (str | Path, optional): Path to an additional file to copy to the training directory. Defaults to None.
        ...
        # Copy the additional file
        # if additional_file_path:
        #     save_dir = Path(self.trainer.save_dir)  # Get the save directory from the trainer
        #     additional_file_path = Path(additional_file_path)
        #     destination_path = save_dir / additional_file_path.name
        #     import shutil
        #     shutil.copy(additional_file_path, destination_path)
        #     LOGGER.info(f"Copied additional file to {destination_path}")

        import shutil
        if additional_file_path:
            for file_path in additional_file_path:
                file_path = Path(file_path)  # Convert each file path to Path object
                save_dir = Path(self.trainer.save_dir)
                destination_path = save_dir / file_path.name  # Define the destination path
                
                try:
                    shutil.copy(file_path, destination_path)  # Copy the file
                    LOGGER.info(f"Copied file {file_path} to {destination_path}")
                except Exception as e:
                    LOGGER.error(f"Failed to copy {file_path} to {destination_path}: {e}")
